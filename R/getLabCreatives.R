#' Get creatives by labels
#' 
#' @description Retrieves creatives by labels under given account.
#' 
...
#' creatives <- getLabCreatives(sample(accounts$id, 1), labels = labs$id, 
#'                              token = TK)
#' }
#' 
...
getLabCreatives <- function(account.id, labels, fields = c("id", "name"), 
  ...
  uri <- paste0("https://graph.facebook.com/v2.6/",
                account.id, "/adcreativesbylabels?fields=", fields, 
