#' List Advertiseable Applications
#' 
#' @description Fetches the list of advertiseable applications under a 
#' given account.
#' 
#' @param id
...
#' @examples 
#' \dontrun{
#' (ad_apps <- listAdApps(id = "act_123456789012345", token = "XXXXXX"))
#' }
#' 
#' @author John Coene \email{john.coene@@cmcm.com}
#' 
#' @export
listAdApps <- function (id, token, n = 100, verbose = FALSE, limit = 100) {
  
  fb_data <- findObjects(id = id, token = token, fields = NULL, 
                         n = n, verbose = verbose, 
                         object = "advertisable_applications", FUN = NULL, 
                         limit = limit)
