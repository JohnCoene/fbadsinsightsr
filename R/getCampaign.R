...
#' 
#' @details This function refers to the following API call 
...
#' data <- getCampaign(campaign.id = rand_id, token = fbOAuth,
#'                     date.preset = date)
#' }
#' 
#' @export
...
getCampaign <- function(campaign.id, token, fields = "default", n = 100, 
  ...
  fb_data <- getAny(id = campaign.id, token = token, fields = fields,
